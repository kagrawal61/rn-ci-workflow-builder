...
export function validateWorkflowSchema(config: WorkflowConfig): WorkflowConfig {
  ...
  const validatedConfig: WorkflowConfig = {
    kind: config.kind,
    options: config.options ? validateWorkflowOptionsSchema(config.options) : {},
  };
  
  // Validate preset-specific options
  if (config.kind === 'build' && config.options) {
    const options = config.options as WorkflowOptions & { build?: BuildOptions };
    if (options.build) {
      (validatedConfig.options as WorkflowOptions & { build?: BuildOptions }).build = 
...
function validateWorkflowOptionsSchema(options: WorkflowOptions): WorkflowOptions {
  ...
  if (options.cache !== undefined) {
    ...
    validatedOptions.cache = {
      enabled: options.cache.enabled !== undefined ? !!options.cache.enabled : true,
    };
  ...
  if (options.concurrency !== undefined) {
    if (typeof options.concurrency !== 'object' || options.concurrency === null) {
    ...
    validatedOptions.concurrency = {
      ...
      cancelInProgress: options.concurrency.cancelInProgress !== undefined 
 ...
 */
function validateTriggers(triggers: WorkflowOptions['triggers']): NonNullable<WorkflowOptions['triggers']> {
  if (!triggers) {
  ...
  if (triggers.push !== undefined) {
    ...
    if (triggers.push.ignorePaths !== undefined) {
      ...
      validatedTriggers.push.ignorePaths = triggers.push.ignorePaths.map(path => {
        if (typeof path !== 'string') {
  ...
  if (triggers.pullRequest !== undefined) {
    ...
    if (triggers.pullRequest.branches !== undefined) {
      ...
      validatedTriggers.pullRequest.branches = triggers.pullRequest.branches.map(branch => {
        if (typeof branch !== 'string') {
          throw new Error('Branch name must be a string');
        }
        return branch;
  ...
  if (triggers.pullRequestTarget !== undefined) {
    ...
    if (triggers.pullRequestTarget.branches !== undefined) {
      ...
      validatedTriggers.pullRequestTarget.branches = triggers.pullRequestTarget.branches.map(branch => {
        if (typeof branch !== 'string') {
          throw new Error('Branch name must be a string');
        }
        return branch;
  ...
  if (triggers.schedule !== undefined) {
    ...
    validatedTriggers.schedule = triggers.schedule.map(scheduleItem => {
      ...
      if (!scheduleItem.cron || typeof scheduleItem.cron !== 'string') {
        throw new Error('Schedule item must have a cron expression as a string');
      }
 ...
 */
function validateEnum<T>(value: T, allowedValues: readonly string[], fieldName: string): T {
